<template>
    <div>
        <el-form ref="form" :model="video" :rules="rules" @keyup.enter.native="search">
            <el-form-item style="display:flex" label="视频名称" prop="data">
    ...
    methods: {
        ...
        search(){
            this.$refs.form.validate((valid) => {
                if (valid) {
                    this.$axios.post(`/video/virtual/name/${this.video.data}`).then(res=>{
        ...
        reset(){
            this.$axios.get('/video/virtualMachine').then(res=>{
            this.videos=res.data;
            this.total=this.videos.length;
        })
            this.video.data=""
