<template>
    <div>
        <el-form @submit.native.prevent="saveArticle" ref="form" :model="article" label-width="80px" @keyup.enter.native="submitForm('form')">
            <el-form-item label="实验日期" prop="date" style="width:300px">
                <el-input v-model="article.date"></el-input>
            </el-form-item>
            <el-form-item label="实验名称" prop="name" style="width:300px">
                <el-input v-model="article.name"></el-input>
            </el-form-item>
            <el-form-item label="实验内容" prop="content" style="width:1000px">
...
export default {
    ...
    methods: {
        saveArticle(){
            this.$axios.put('/lab/'+this.$route.params.id,this.article).then(()=>{
                this.$message({
                    type: 'success',
                    message: '保存成功!'
                })
    ...
    created(){
        this.$axios.get('/lab/'+this.$route.params.id).then(res=>{
