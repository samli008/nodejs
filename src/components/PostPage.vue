<template>
    <div>
        ...
        <el-table :data="posts.slice((page_num-1)*page,page_num*page)" style="width: 100%">
            <el-table-column prop="name" label="文档名称" width="110"></el-table-column>
...
export default {
    ...
    methods: {
        ...
        edit(scope){
            this.$router.push("/PostEdit/"+scope.row._id)
        },
        create(){
        ...
        del(scope){
            this.$confirm('确认删除?', '提示', {
                ...
                this.$axios.delete("/doc/"+scope.row._id);
                this.posts.splice(scope.$index,1);
                this.$message({
                    type: 'success',
                    message: '删除成功!'
            });
        ...
        search(){
            this.$refs.form.validate((valid) => {
                if (valid) {
                    this.$axios.post(`/doc/${this.article.value}/${this.article.data}`).then(res=>{
                        this.posts=res.data
